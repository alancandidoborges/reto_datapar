...
var
  StrSql: string;
begin
  StrSql := 'INSERT INTO ElementosPedido(COD_PRODUCTO, VALOR_UNITARIO, VALOR_TOTAL, CANTIDAD, PEDIDO)';
  ...
  StrSql := StrSql + AElementosPedido.Pedido.ToString           + ') ';
  FQuery.close;
  FQuery.SQL.Clear;
...
function TElementosPedidoDAO.LoadGrid(APedido: Integer): TFDQuery;
var
  StrSql: String;
begin
  try
    StrSql := 'select item.pedido as cod_venda, item.id as codigo, producto.id as cod_producto, ';
    StrSql := StrSql + ' producto.descripcion, item.cantidad, item.valor_unitario as Precio, item.valor_total as PrecioTotal, item.TipoOperacion ';
    StrSql := StrSql + ' from  elementospedido item, producto producto where item.cod_producto = producto.id ';
    StrSql := StrSql + ' and item.pedido = ' + APedido.ToString;
    FQuery.Open(StrSql);
    Result := FQuery;
  finally
    FQuery.Close;
