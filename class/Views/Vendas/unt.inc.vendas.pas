...
begin
  ...
  begin
    mensagem := 'El campo [Cliente] no debe estar vacío.';
    Application.MessageBox(PChar(mensagem), 'Información', MB_OK + MB_ICONWARNING);EdtCliente.SetFocus;
    Result := False;
    Abort;
  end;
  ...
  begin
    mensagem := 'El campo [Fecha - Entrega] no debe estar vacío.';
    Application.MessageBox(PChar(mensagem), 'Información', MB_OK + MB_ICONWARNING);EdtCliente.SetFocus;
    Result := False;
    Abort;
  end;
  ...
  begin
    mensagem := 'No hay artículos en oferta para este pedido.';
    Application.MessageBox(PChar(mensagem), 'Información', MB_OK + MB_ICONWARNING);EdtCliente.SetFocus;
    Result := False;
    Abort;
  end;
...
begin
  if Consiste() then
  begin
    confExcl:= Application.MessageBox('¿ Confirmas la Grabación?','Atenção',MB_YESNO + MB_DEFBUTTON1 + MB_ICONINFORMATION);
